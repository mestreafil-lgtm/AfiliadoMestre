        ...
        function amGenSessionId() {
            ...
            // Fallback UUID v4 quando crypto.randomUUID não existe (browsers antigos).
        ...
        function amSessionId() {
            try {
                const now = Date.now();
                const raw = sessionStorage.getItem(AM_SESSION_KEY);
                const parsed = raw ? JSON.parse(raw) : null;
                if (parsed && parsed.id && Number.isFinite(parsed.last) && now - parsed.last <= AM_SESSION_IDLE_MS) {
                    parsed.last = now;
                    sessionStorage.setItem(AM_SESSION_KEY, JSON.stringify(parsed));
                    return parsed.id;
                }
                const fresh = { id: amGenSessionId(), last: now };
                sessionStorage.setItem(AM_SESSION_KEY, JSON.stringify(fresh));
                return fresh.id;
            } catch (_) {
                // sessionStorage bloqueado (Safari private, embed): devolve um id efêmero
                // que ao menos correlaciona eventos dentro da mesma chamada.
...
async function loadOffersFromSupabase(opts = {}) {
        ...
        function productCardHTML(p, { index = 0, section = 'destaque' } = {}) {
            ...
            return `
                <div class="bg-white rounded-xl overflow-hidden border border-slate-100 hover:border-shopee-orange hover:shadow-md transition flex flex-col h-full relative group min-w-0">
                    <div onclick="openProductModal(${p.id}, '${section}', ${Number.isFinite(index) ? index : 'null'})" class="cursor-pointer flex flex-col flex-1 min-w-0">
                    ${disc ? `
        ...
        // Timestamp do último ProductOpen: usado por closeProductModal pra medir duration_ms.
        let productModalOpenedAt = 0;
        let productModalOpenedFor = null;

        // Abre o popup com as informações do produto para o cliente
        function openProductModal(id, section = null, index = null) {
        ...
        function closeProductModal() {
            ...
            const card = document.getElementById('modal-card');
            if (!modal || !card) return;
            // Dispara ANTES da animação pra não perder o evento se a aba fechar em seguida.
            if (productModalOpenedAt && productModalOpenedFor != null) {
                const durationMs = Math.max(0, Date.now() - productModalOpenedAt);
                amEventTrack("ProductClose", {
                    product_id: Number(productModalOpenedFor),
                    duration_ms: durationMs,
                });
                productModalOpenedAt = 0;
                productModalOpenedFor = null;
            }
