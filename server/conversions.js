...
async function summary({ from, to, days = 30, onlyMeuSite = true } = {}) {
  ...
  const rows = await safeGet(
    `/conversions?select=conversion_id,purchase_time,order_status,fraud_status,shop_id,shop_name,item_id,item_name,item_price,actual_amount,net_commission,total_commission,sub_id1,sub_id2,sub_id3,validated&${scopeFilter}&order=purchase_time.desc&limit=5000`
  ) || [];

  const list = Array.isArray(rows) ? rows : [];
  const { listKnownCampaignNames } = require("./supabase");
  const knownCampaigns = await listKnownCampaignNames().catch(() => new Set());

  function campaignKeyFromRow(r) {
    const s1 = sanitizeSubId(r.sub_id1, "");
    const s3 = sanitizeSubId(r.sub_id3, "");
    if (s1 && s1 !== SITE_SUBID && (knownCampaigns.has(s1) || (!r.sub_id2 && !r.sub_id3))) {
      return s1;
    }
    return s3 || null;
  }
  ...
  for (const r of list) {
    ...
    if (r.sub_id3 || r.sub_id1) {
      const key = campaignKeyFromRow(r);
      if (key) {
        const prev = byCampaign.get(key) || { campaign: key, gross: 0, net: 0, orders: 0 };
        prev.gross += moneyG;
        prev.net += moneyN;
        prev.orders += 1;
